...
Use the repository-root Make targets. `lab-up` delegates to the pinned
`ffworker/proxmox-lab` CKA scenario, which runs Terraform, the Ansible kubeadm
bootstrap, local API tunnel, and Ready checks. `lab-down` first verifies
Terraform state and live VM ownership markers, then destroys only VMs 320 and
321 (`cka-cp01` and `cka-worker01`).
